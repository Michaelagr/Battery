        ...
        for idx in range(len(df_case_4)):
            ...
            # 3. Arbitrage and Emergency Charging
             # Threshold > net_load_kwh > 0
                ...
                if current_price <= low_price_threshold and battery_soc_kwh < battery_capacity_kwh and arbitrage_enabled:
                    # NEVER exceed peak shaving threshold for arbitrage!
                ...
                if (current_price >= high_price_threshold and action_taken == 0 and
                    ...
                    grid_import_kwh_interval > 0) and arbitrage_enabled == True:
                    
                    # Arbitrage can only discharge down to min_soc_kwh, respecting the minimum SoC
                    # (it uses the "other uses" capacity, not the peak-shaving reserved capacity)
                    required_reserve = max(min_soc_kwh, peak_reserve_soc)
...
def main():
    ...
    if available_files:
        ...
        st.sidebar.info("Keine .xlsx Dateien im /input_customers Ordner gefunden.")
